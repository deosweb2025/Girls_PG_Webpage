...
const Amenities = () => {
  return (
    <section id="amenities" className="py-20 lg:py-28 bg-[#FAF7F4] relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {siteData.amenities.map((amenity, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`rounded-3xl p-7 transition-all duration-300 group relative border ${
                  isFeatured
                    ? 'bg-gradient-to-br from-white via-white to-[#F6F1EA] border-[#E8DFD5] shadow-lg md:col-span-2 lg:col-span-1'
                    : 'bg-white border-[#E8DFD5] shadow-xs hover:shadow-md hover:border-[#832B4C]/40'
                }`}
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="w-12 h-12 rounded-2xl bg-[#832B4C]/10 text-[#832B4C] flex items-center justify-center group-hover:bg-[#832B4C] group-hover:text-white transition-colors duration-300">
                    <Icon className="w-6 h-6" />
                  </div>
                  
                  <span className="text-[11px] font-extrabold uppercase px-3 py-1 rounded-full bg-[#FAF7F4] border border-[#E8DFD5] text-[#832B4C] tracking-wide">
                    {amenity.badge}
                  </span>
                </div>

                <h3 className="text-lg font-bold text-[#1E1B18] mb-2 group-hover:text-[#832B4C] transition-colors">
                  {amenity.title}
                </h3>
                ...
                <div className="mt-5 pt-3 border-t border-[#F3EDE6] flex items-center text-xs font-semibold text-[#832B4C] opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <div className="mt-12 p-6 sm:p-8 rounded-3xl bg-gradient-to-r from-[#4A1D2F] via-[#832B4C] to-[#4A1D2F] text-white flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl">
          <div className="max-w-2xl">
            <span className="text-xs font-bold uppercase tracking-widest text-[#D48B68]">
